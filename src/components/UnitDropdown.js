...
const UNITS = [
  'pcs', 'kg', 'g', 'mg', 'l', 'ml',
  'pack', 'box', 'bottle', 'dozen', 'pair', 'set',
];
 ...
 */
const UnitDropdown = ({ value, onChange, error }) => {
  const [focused, setFocused] = useState(false);

  const data = UNITS.map((u) => ({ label: u, value: u }));
  ...
  return (
    <View style={styles.wrap}>
      ...
      <View style={[styles.iconBox, { borderColor }]}>
        <Icon name="scale-outline" size={rfs(15)} color={iconColor} />
      </View>
